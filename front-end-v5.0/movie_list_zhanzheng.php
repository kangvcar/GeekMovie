<?php 
    $hot_MovieSql = "SELECT mid, mname, mimgurl, mtype, mstar FROM movie WHERE mtype LIKE '战争片' LIMIT 6;";
    $result18 = $conn->query($hot_MovieSql);
    if ($row = $result18->num_rows > 0) {
        while ($row = $result18->fetch_row()) {
          print <<<EOG
