<?php 
    $hot_MovieSql = "SELECT mid, mname, mimgurl, mtype, mstar FROM movie WHERE mtype LIKE '动作片' LIMIT 6;";
    $result12 = $conn->query($hot_MovieSql);
    if ($row = $result12->num_rows > 0) {
        while ($row = $result12->fetch_row()) {
          print <<<EOG
