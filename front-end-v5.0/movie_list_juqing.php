<?php 
    $hot_MovieSql = "SELECT mid, mname, mimgurl, mtype, mstar FROM movie WHERE mtype LIKE '剧情片' LIMIT 6;";
    $result13 = $conn->query($hot_MovieSql);
    if ($row = $result13->num_rows > 0) {
        while ($row = $result13->fetch_row()) {
          print <<<EOG
