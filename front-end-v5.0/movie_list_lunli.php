<?php 
    $hot_MovieSql = "SELECT mid, mname, mimgurl, mtype, mstar FROM movie WHERE mtype LIKE '伦理片' LIMIT 6;";
    $result16 = $conn->query($hot_MovieSql);
    if ($row = $result16->num_rows > 0) {
        while ($row = $result16->fetch_row()) {
          print <<<EOG
