<?php 
    $hot_MovieSql = "SELECT mid, mname, mimgurl, mtype, mstar FROM movie WHERE mtype LIKE '恐怖片' LIMIT 6;";
    $result15 = $conn->query($hot_MovieSql);
    if ($row = $result15->num_rows > 0) {
        while ($row = $result15->fetch_row()) {
          print <<<EOG
